...
export default function AdminConfigPage() {
  ...
  return (
    <div className="p-6 space-y-8 max-w-6xl mx-auto font-sans">
      ...
      <Section title="Información de Contacto" icon={<Contact className="w-5 h-5 text-gray-500" />}>
      ...
      {/* Redes sociales */}
      <Section title="Redes Sociales" icon={<Share2 className="w-5 h-5 text-gray-500" />}>
      ...
      {/* Landing page */}
      <Section title="Información de la Página Inicial" icon={<LayoutTemplate className="w-5 h-5 text-gray-500" />}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
        ...
        <div className="aspect-[4/2.5] max-w-4xl mx-auto overflow-hidden rounded-xl shadow-lg flex flex-col lg:flex-row font-[Poppins]">
          <div className="w-full lg:w-1/2 bg-gray-200 flex flex-col justify-center pl-10 pr-6 py-8">
            <h1 className="text-[#006881] font-bold text-[2.1em] leading-tight">
              {config.landingTitle || "Mejorando la Calidad de Vida de Nuestros Mayores"}
            </h1>
            <p className="text-[#4B5563] text-[1.2em] mt-3">
              {config.landingSubtitle || "Brindamos compañía, apoyo y cuidado personalizado para adultos mayores."}
            </p>
            <div className="flex flex-col sm:flex-row text-[1em] gap-4 mt-5">
              <div className="px-4 py-4 bg-a7 text-white font-semibold hover:bg-a7/80 rounded-lg transition">Explorar Servicios</div>
              <div className="px-4 py-4 bg-secondary2 text-white font-semibold hover:bg-secondary2/80 rounded-lg transition">Crear cuenta</div>
            </div>
          </div>
          <div
            ...
            style={{
              backgroundImage: `
                linear-gradient(to right, hsl(220, 13%, 91%) 0%, rgba(229, 231, 235, 0.31) 25%, rgba(0, 0, 0, 0) 50%),
...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-white shadow rounded-xl p-5 space-y-4">
      <h2 className="font-semibold text-lg text-gray-800 flex items-center gap-2">{icon} {title}</h2>
      {children}
    </div>
  )
}